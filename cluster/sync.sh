...
pods_ready_wait() {
  ...
  max_retries=12
  retry_counter=0
  echo "Waiting for kubemacpool recourses to be ready ..."
  while [[ "$(./cluster/kubectl.sh wait pod -n kubemacpool-system --all --for=condition=Ready --timeout=1m)" = $wait_failed ]] && [[ $retry_counter -lt $max_retries ]]; do
    sleep 5s
