...
MACPOOL_PROVIDER=${MACPOOL_PROVIDER:-${PROVIDER}}

source ${MACPOOL_PATH}hack/config-default.sh

# Allow different providers to override default config values
test -f "hack/config-provider-${MACPOOL_PROVIDER}.sh" && source hack/config-provider-${MACPOOL_PROVIDER}.sh
