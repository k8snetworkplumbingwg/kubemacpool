...
func macAddressesChanged(oldVMI, newVMI *kubevirtv1.VirtualMachineInstance) bool {
	return !maps.Equal(supportedMACSet(oldVMI), supportedMACSet(newVMI))
}

func supportedMACSet(vmi *kubevirtv1.VirtualMachineInstance) map[string]struct{} {
	macs := make(map[string]struct{})
	for _, mac := range supportedMACsFromVMI(vmi) {
		macs[mac] = struct{}{}
	}
	return macs
}
