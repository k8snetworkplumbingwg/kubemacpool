...
var _ = Describe("Predicates", func() {
	...
	Describe("Update", func() {
		...
		Context("when MAC addresses change", func() {
			It("should trigger reconciliation when a supported MAC is added", func() {
			...
			It("should trigger reconciliation when a supported MAC is removed", func() {
			...
			It("should trigger reconciliation when a supported MAC value changes", func() {
				oldVMI := newVMI("test-ns", "test-vmi", withPhase(kubevirtv1.Running), withMACs(testMAC1))
			...
			It("should not trigger reconciliation when only unsupported pod-network MACs change", func() {
				oldVMI := newVMI("test-ns", "test-vmi",
					withPhase(kubevirtv1.Running),
					withUnsupportedPodNetworkMAC("pod", testMAC1))
				newVMI := newVMI("test-ns", "test-vmi",
					withPhase(kubevirtv1.Running),
					withUnsupportedPodNetworkMAC("pod", testMAC2))
			...
			It("should trigger reconciliation when spec drops the last supported NIC with the same status MAC", func() {
				oldVMI := newVMI("test-ns", "test-vmi", withPhase(kubevirtv1.Running), withMACs(testMAC1))
				newVMI := oldVMI.DeepCopy()
